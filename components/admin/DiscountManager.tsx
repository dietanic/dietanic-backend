...
import { Trash2, Tag, Plus, Percent, DollarSign, AlertCircle, Edit } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        <div className="flex justify-between items-center">
            ...
            <button 
                onClick={() => { editingDiscount ? handleCancel() : setIsAdding(!isAdding) }}
        ...
        {isAdding && (
            <form onSubmit={handleSaveDiscount} className="bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-inner animate-scale-in">                <h4 className="text-lg font-bold text-gray-800 mb-4">{editingDiscount ? 'Edit Coupon' : 'Create New Coupon'}</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">                    <div>
                    ...
                    <div className="flex gap-4">                        <div className="flex-1">                            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Value</label>
                            <input 
                                type="number" 
                        ...
                        <div className="w-32">                            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Type</label>
                            <select 
                    ...
                    <div className="lg:col-span-2 flex items-end">                        <button className="bg-gray-900 text-white px-8 py-2.5 rounded-full font-bold hover:bg-black transition-colors w-full sm:w-auto">                           {editingDiscount ? 'Update Promo Code' : 'Publish Promo Code'}
                        </button>
                    </div>
                </div>
            </form>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">            <div className="overflow-x-auto">                <table className="min-w-full divide-y divide-gray-200">                    <thead className="bg-gray-50">                        <tr>                            <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">Coupon Detail</th>
                    ...
                    <tbody className="divide-y divide-gray-100">                        {discounts.map(d => (
                            <tr key={d.id} className="hover:bg-gray-50 transition-colors group">                                <td className="px-6 py-5">                                    <div className="flex items-center gap-3">                                        <div className="p-2 bg-brand-50 text-brand-600 rounded-xl group-hover:scale-110 transition-transform">                                            <Tag size={20} />
                                        </div>
                                        <div>
                                            <span className="font-mono font-black text-gray-900 text-lg">{d.code}</span>
                                            <div className="flex items-center gap-2 mt-0.5">                                                <span className={`w-2 h-2 rounded-full ${d.isActive ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                                                <span className="text-[10px] font-bold text-gray-400 uppercase">{d.isActive ? 'Live & Active' : 'Inactive'}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5">                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${                                        d.type === 'percentage' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'
                                    }`}>                                        {d.type === 'percentage' ? <Percent size={12}/> : <DollarSign size={12}/>}
                                        {d.type === 'percentage' ? `${d.value}% Discount` : `₹${d.value} Off`}
                                    </span>
                                </td>
                                <td className="px-6 py-5">                                    <div className="space-y-1">                                        <div className="text-sm text-gray-700 font-medium">Valid on: <span className="text-gray-900 font-bold">{d.applicableCategory || 'All Items'}</span></div>                                        <div className="text-xs text-gray-400">                                            {d.minPurchaseAmount ? `Min Spend: ₹${d.minPurchaseAmount}` : 'No minimum spend'}
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5 text-right">                                   <button 
                            ...
                            <tr>                                <td colSpan={4} className="px-6 py-20 text-center text-gray-400">                                    <Tag size={48} className="mx-auto mb-4 opacity-10" />
        ...
        <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 flex items-start gap-4">            <div className="p-2 bg-white rounded-full text-blue-500 shadow-sm">                <AlertCircle size={20}/>
            </div>
            <div>
